#include "EDC.h"
#include <immintrin.h>  // For carry-less multiplication

#define CRC_HD_MAX 5
#define CRC_TABLE_LENGTH 8
#define CRC_POLY 0x04C11DB7
#define CRC_POLY_REVERSE 0xEDB88320

namespace CRC       // LSB-first CRC
{
    uint32_t CRC32_poly = CRC_POLY;  // 0x04C11DB7
    ...
    uint32_t CRC32_Naive(uint32_t* ptr, size_t length)
    {
        uint32_t crc = 0xFFFFFFFF;      // REFIN
        // uint32_t crc = 0;      // REFIN
        ...
        for (size_t i = 0; i < length * sizeof(uint32_t); i++)
        {
            ...
            {
                // if (crc & 1)
                //     crc = (crc >> 1) ^ CRC_POLY_REVERSE;
                // else
                //     crc = crc >> 1;

                crc = (crc >> 1) ^ (-int32_t(crc & 1) & CRC_POLY_REVERSE);      // faster
                // crc = (crc >> 1) ^ (crc & 1) * CRC_POLY_REVERSE;
            }
        }
        return ~crc;    // REFOUT
        // return crc;    // REFOUT
    }
    uint32_t CRC32_Naive_Reverse(uint32_t* ptr, size_t length)
    {
        // uint32_t crc = 0;      // REFIN
        uint32_t crc = 0xFFFFFFFF;      // REFIN
        unsigned char* data = (unsigned char*)ptr;
    ...
    // ---- LUT algorithm ----
    //   Single table - slicing-by-n (n=8 by default)
    bool CRC_Table_Flag = false;
    uint32_t CRC_Table[1 << CRC_TABLE_LENGTH];
    void Make_CRC_Table(uint32_t CRC_Poly, int nbits)       // bitwise (below 32)
    {
        uint32_t CRC_Reverse = 0;
        for (int i = 0; i < nbits; i++)
            if (CRC_Poly & (1 << i))
                CRC_Reverse ^= (1 << (nbits - i - 1));

        for (int i = 0; i < (1 << CRC_TABLE_LENGTH); i++)       // all 8-bit values
        {
            uint32_t temp = i;
            for (int j = 0; j < CRC_TABLE_LENGTH; j++)          // 8 bit offset
            {
                if (temp & 1)
                    temp = (temp >> 1) ^ CRC_Reverse;
                else
                    temp = temp >> 1;
            }
            CRC_Table[i] = temp;
    ...
    inline uint32_t CRC32_Table_Raw(uint32_t* ptr, size_t length, uint32_t crc_init = 0xFFFFFFFF)
    {
        uint32_t crc = crc_init;
        unsigned char* data = (unsigned char*)ptr;
        for (size_t i = 0; i < length * sizeof(uint32_t); i++)
            crc = (crc >> CRC_TABLE_LENGTH) ^ CRC_Table[(crc ^ data[i]) & 0xFF];
        return crc;
    }
    uint32_t CRC32_Table(uint32_t* ptr, size_t length)
    {
        if (!CRC_Table_Flag)
            Make_CRC_Table(CRC_POLY, WORD_SIZE);
        // uint32_t crc = 0xFFFFFFFF;
        // unsigned char* data = (unsigned char*)ptr;
        // for (size_t i = 0; i < length * WORD_SIZE / CRC_TABLE_LENGTH; i++)
        //     crc = (crc >> CRC_TABLE_LENGTH) ^ CRC_Table[(crc ^ data[i]) & 0xFF];
        uint32_t crc = CRC32_Table_Raw(ptr, length, 0xFFFFFFFF);
        return ~crc;
    }

    //   Parallel tables - k times slicing-by-8 (k=16 by default)
    const size_t MaxSlice = 16;     // Instruction Level Parallelism
    ...
    uint32_t CRC32_Parallel8(uint32_t* ptr, size_t length)
    {
        uint32_t crc = 0xFFFFFFFF; // same as previousCrc32 ^ 0xFFFFFFFF
        const uint32_t* current = ptr;
        length *= sizeof(uint32_t) / sizeof(uint8_t); // convert length to bytes
    ...
    uint32_t CRC32_Parallel16(uint32_t* ptr, size_t length)
    {
        uint32_t crc = 0xFFFFFFFF; // previous CRC = 0
        // uint32_t crc = 0; // previous CRC = 0
    ...
    // ---- CL intrinsics ----
    uint32_t CL_Table[MAX_LEN] = {0};
    bool CL_Table_Initialized = false;
    uint32_t CRC32_F_Delta;
    uint32_t ENDIAN(uint32_t val)
    {
    ...
    void Init_CL_Table(size_t length = MAX_LEN)
    {
        uint32_t temp[length] = {0};
        for (size_t i = 0; i < length; i++)
        {
            temp[i] = 1 << 31; // REFLECT32(0x00000001);
            // temp[i] = 1;
            CL_Table[i] = REFLECT32(CRC32_Table_Raw(temp, length, 0));
            // printf("CL_Table[%zu]: %08X\n", i, CL_Table[i]);
            temp[i] = 0;
        }
        return;
    }
    bool CL_log = false;    
    uint32_t CRC32_CL_Table(uint32_t* ptr, size_t length)
    {
        if (!CL_Table_Initialized)
        {
            Init_CL_Table(length);
            CL_Table_Initialized = true;
        }
        // uint64_t res = 0xFFFFFFFF;
        uint64_t res = 0;
        for (size_t i = 0; i < length; ++i) {
            // __m128i a_vec = _mm_cvtsi32_si128(ptr[i]);
            __m128i a_vec = _mm_cvtsi32_si128(REFLECT32(ptr[i]));
            __m128i b_vec = _mm_cvtsi32_si128(CL_Table[i]);

            __m128i clmul_res = _mm_clmulepi64_si128(a_vec, b_vec, 0x00);

            res ^= _mm_cvtsi128_si64(clmul_res);
        }
        uint32_t temp[2] = { (uint32_t)(res >> 32), (uint32_t)(res & 0xFFFFFFFF) };
        // uint32_t temp[2] = { ENDIAN(res >> 32), ENDIAN(res & 0xFFFFFFFF) };
        
        // if (CL_log)
        //     printf("CL intermediate CRC: %08X %08X\n", temp[0], temp[1]);
        res = CRC32_Table_Raw(temp, 2);
    ...
    // ---- HD Search ----
    uint32_t HD[CRC_HD_MAX] = {0};
    // size_t Combination[CRC_HD_MAX] = {0};
    uint32_t* Error_Table;// [MAX_LEN * 32] = {0};

    void InitErrorTable(size_t length)
    {
        Error_Table = (uint32_t*)malloc(length * 32 * sizeof(uint32_t));

        uint32_t* temp = (uint32_t*)malloc(length * 32 * sizeof(uint32_t));
        uint32_t temp_value = CRC32_Table(temp, length);        // base value

        // for (int i = length * 32 - 1; i >= 0; i--) // start from rightmost bit
        for (size_t i = 0; i < length * 32; i++)
        {
            size_t index = i / WORD_SIZE;
            size_t position = i % WORD_SIZE;
            temp[index] = (1u << position);
            Error_Table[i] = CRC32_Table(temp, length) ^ temp_value;
            temp[index] = 0; // reset
        }
        free(temp);
    }
    void generate_combination(int num, size_t lower_bound, int total, size_t length, uint32_t remainder)
    {
        if (lower_bound >= length * 32)  
            return;
        if (num == total)
        {
            if (remainder == 0)
        ...
        {
            uint32_t error_value = Error_Table[i];
            generate_combination(num + 1, i + 1, total, length, remainder ^ error_value);
        }
    }
    void HD_Search_Table(size_t length)
    {
        InitErrorTable(length);
        for (int i = 2; i < CRC_HD_MAX; i++)    // number of errors
        {
            generate_combination(1, 1, i, length, Error_Table[0]);      // including the codeword itself, otherwise + 1
            if (HD[i] != 0)
                break;
        }
        for (int i = 2; i < CRC_HD_MAX; i++)
            printf("HD[%d]: %u\n", i, HD[i]);
    }
    ...
    void perf_test(size_t length)
    {
        ...
        printf("  Naive: ");
        run_perf_test(CRC32_Naive, length, A);
        printf("  Table: ");
        run_perf_test(CRC32_Table, length, A);
        printf("  Parallel8: ");
        run_perf_test(CRC32_Parallel8, length, A);
        printf("  Parallel16: ");
        run_perf_test(CRC32_Parallel16, length, A);
        printf("  CL: ");
        run_perf_test(CRC32_CL_Table, length, A);
    ...
    void functional_test(size_t length)
    {
        ...
        uint32_t CRC32_F_Delta = CRC32_Table_Raw(A, length);
        for (size_t i = 0; i < length; i++)
            A[i] = i;
            
        printf("CRC combine: CRC_0 = %X, Delta_CRC = %X => CRC_F = %X, CRC_0 XOR Delta_CRC = %X\n",
            CRC32_Table_Raw(A, length, 0),
            CRC32_F_Delta,
            CRC32_Table_Raw(A, length, 0xFFFFFFFF),
            CRC32_Table_Raw(A, length, 0) ^ CRC32_F_Delta);


        uint32_t result_Naive = CRC32_Naive(A, length);
        uint32_t result_Table = CRC32_Table(A, length);
        uint32_t result_Parallel8 = CRC32_Parallel8(A, length);
        uint32_t result_Parallel16 = CRC32_Parallel16(A, length);
        CL_log = true;
        uint32_t result_CL = CRC32_CL_Table(A, length);

        printf("CRC Naive reverse:    %X\n", REFLECT32(CRC32_Naive_Reverse(A, length)));

        if (result_Naive != result_Table || result_Table != result_Parallel8 || result_Parallel8 != result_Parallel16 || result_Parallel16 != result_CL)
            printf("Functional: incorrect! %X %X %X %X %X\n", result_Naive, result_Table, result_Parallel8, result_Parallel16, result_CL );
        else
            printf("Functional: correct! CRC32 = %X\n", result_Naive);
        free(A);
    }
